...
LeaderboardWindow::LeaderboardWindow(int r, int c){
    
    _leaderboard_filename = "data/leaderboard.txt";
    _rows = r;
    _cols = c;
...
void LeaderboardWindow::init_fonts(){
    if (! _text_font.loadFromFile("assets/font.ttf")){
        std::cout << "error loading font\n";
    }
}

void LeaderboardWindow::init_window(){

    render_window.create(sf::VideoMode(_width, _height), "leaderboard", sf::Style::Close);
}

void LeaderboardWindow::init_leaderboard_scores(){
...
void LeaderboardWindow::init_title(){
    // sets up text!
    sf::Vector2f title_pos(_width/2, (_height/2) - 120);

}

void LeaderboardWindow::set_leaderboard_text(){
...
void LeaderboardWindow::display_leaderboard(){
    init_window();
...
bool LeaderboardWindow::has_score(std::string username, int min, int sec){
...
void LeaderboardWindow::draw_all(){
...
void LeaderboardWindow::setup_text(sf::Text &text, std::string message, sf::Font &font, bool bold, bool underlined, sf::Color color, int size, sf::Vector2f position){
...
void LeaderboardWindow::setTextPos(sf::Text &text, float x, float y){
...
void LeaderboardWindow::read_leaderboard(){
...
void LeaderboardWindow::write_leaderboard(){
...
void LeaderboardWindow::print_scores(){
    for (std::size_t i = 0; i < _scores_leaderboard.size(); i += 1){
        _scores_leaderboard[i].print();
    }
}

void LeaderboardWindow::sort_scores(){
...
void LeaderboardWindow::add_score(std::string name, int min, int sec){
