...
int main(){
    ...
    minesweeper::ui::WelcomeWindow welcome_window(rows, cols);
    ...
    minesweeper::ui::GameWindow game_window(rows, cols, mines, username);
...
void read_config(int &rows, int &cols, int &mines){
    ...
    std::ifstream config_fin("config/board.cfg");
    std::string buffer;
